...
- **双通道支付**：支持 XorPay（第三方聚合）和 MbdPay（面包多，0 手续费），通过环境变量一键切换商户充值通道
...
```env
# 支付渠道：mbdpay（推荐，0 手续费）或 xorpay
PAY_CHANNEL=mbdpay

# MbdPay（https://p.mbd.pub 申请）
MBD_APP_ID=your_mbd_app_id
MBD_APP_KEY=your_mbd_app_key
MBD_NOTIFY_URL=https://yourdomain.com/api/pay/notify

# XorPay（https://xorpay.com 申请，替代方案）
XORPAY_AID=your_aid
XORPAY_APP_KEY=your_app_key
XORPAY_NOTIFY_URL=https://yourdomain.com/api/pay/notify
```
...
│   │   └── 001_init_complete.sql  # 所有表结构（幂等，可直接重跑）
│   └── src/
...
│       │   ├── messages.rs      # 站内信/公告
│       │   ├── payments.rs      # 双通道支付（XorPay / MbdPay）
