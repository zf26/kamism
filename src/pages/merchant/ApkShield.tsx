...
import { apkShieldApi, appsApi } from '../../lib/api';
import { useAuthStore } from '../../stores/auth';
import { Upload, RefreshCw, Download, Clock, CheckCircle2, XCircle, Loader2, Package, Heart } from 'lucide-react';
...
export default function ApkShield() {
  ...
  const loadJobs = () => {
    ...
    setLoading(true);
    apkShieldApi.list(merchantId, page, pageSize).then(res => {
      if (res.data.success) {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!selectedFile) { toast.error('请选择 APK 文件'); return; }
    if (!selectedFile.name.endsWith('.apk')) { toast.error('只支持 .apk 文件'); return; }
    ...
    try {
      ...
      await apkShieldApi.upload(selectedFile, selectedAppId, merchantId, apiKey, selectedFile.name);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      jobsRef.current.filter(j => j.status === 'pending' || j.status === 'processing').forEach(job => {
        apkShieldApi.status(job.job_id).then(res => {
          if (res.data.success) {
  ...
  const handleDownload = async (job: Job) => {
    try {
      const res = await apkShieldApi.download(job.job_id);
      const blob = res.data;
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${job.job_id}_protected.apk`;
      a.click();
  ...
  return (
    <div className="fade-in">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">APK 加固<span className="text-muted">(Beta)</span></h1>
          <p className="page-subtitle">上传 APK，自动注入卡密验证壳，下载加固后安装包</p>
        </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <form onSubmit={handleUpload}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div className="form-group" style={{ margin: 0 }}>
              <label className="form-label">APK 文件 *</label>
              <input
                ref={fileInputRef}
                type="file"
                accept=".apk"
          ...
          <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              加固后会覆盖原签名，请使用自己的签名重新签名，或直接分发加固包
            </span>
          </div>
        </form>
      </div>
              ...
              <tr>
                <td colSpan={5}>
                  <div className="empty-state">
                    ...
                    <div className="empty-state-text">暂无加固记录，上传第一个 APK 开始吧</div>
                  </div>
                </td>
              </tr>
            ) : jobs.map((job, idx) => {
