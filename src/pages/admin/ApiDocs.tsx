...
export default function AdminApiDocs() {
  ...
  useEffect(() => {
    setLoading(true);
    adminApi.getApiKey().then(res => {
      if (res.data.success) setApiKey(res.data.data?.api_key || '');
    }).catch(() => {}).finally(() => setLoading(false));
  }, []);

  const handleRegenerate = () => {
    adminApi.regenerateApiKey().then(res => {
      if (res.data.success) {
  ...
  const copyApiKey = () => {
    if (apiKey) { navigator.clipboard.writeText(apiKey); toast.success('API Key 已复制'); }
  };

  const endpoints = buildEndpoints(apiKey || '<your_api_key>');
    ...
    <div className="fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">集成激活、验证、解绑接口到你的软件（管理员视角）</p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <code className="mono" style={{
          ...
          {loading ? '加载中...' : apiKey || '暂无 Key（请先创建应用）'}
        </code>
        <button className="btn btn-ghost btn-sm" onClick={copyApiKey} disabled={!apiKey}>
          <Copy size={12} /> 复制
        </button>
